...
def parse_command_line
  ...
  opts = Trollop::options do
    version "users_lookup 0.1 (c) 2015 Kevin D. Rau"
    banner USAGE
...
if __FILE__ == $0
  ...
  if ids[0] == "user_ids"
    ...
    msg    = "Retrieving user profile banner information for given user_ids: #{ids[1]}"
  else
    params = { screen_name: ids[1] }
    msg    = "Retrieving user profile banner information for given screen_names: #{ids[1]}"
  end

  data   = { props: input[:props] }
  args   = { params: params, data: data }

  users_data = UsersProfileBanner.new(args)
  puts msg
